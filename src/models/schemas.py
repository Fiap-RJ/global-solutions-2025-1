...
class SensorDataInput(BaseModel):
    ...
    temperature: float = Field(
        ..., gt=-50, lt=100, description="Temperatura em graus Celsius."
    )
    humidity: float = Field(
        ..., gt=0, le=100, description="Umidade relativa do ar em porcentagem."
    )
    timestamp: Optional[str] = Field(
        None, description="Timestamp opcional da medição (formato ISO)."
    )
